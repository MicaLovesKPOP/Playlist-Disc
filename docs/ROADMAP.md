...
- pack/wallet support - Draft 0.2 source schema, validation, 12/24/48/96 capacity metadata, ordered slot resolution, CLI inspection, and deterministic printable HTML pack indexes implemented; physical wallet ergonomics, labels, and artwork remain future work;
...
- live provider plugins (Spotify/Apple/etc. authentication/catalog lookup/materialization) remain product work but are non-blocking for first physical-format validation;
...
- BMW/MINI legacy changer family research implemented: BMW NG I/K-Bus control with a separate changer audio path, BM24/BM54 remote radio-module boundary, MINI R50/R53 K-Bus changer wiring/no-coding retrofit behavior, and a hard separation from later MOST systems; exact bus/changer messages remain capture-gated;
- Fiat/Alfa/Lancia family expansion implemented: multiple Fiat/Lancia/Alfa service documents separate B-CAN radio integration from dedicated ASCI-BUS/CDC data plus analogue audio, with E13-E20 role reuse, Panda-specific electrical/harness evidence, and changer-capacity variants kept head-unit-specific; exact changer frames and CAN observations remain capture-gated;
- P1 family-first compatibility breadth is complete for pre-hardware planning: VAG, BMW/MINI legacy, PSA, Fiat/Alfa/Lancia, Renault, Ford, Opel/Vauxhall, Toyota/Lexus, Honda/Acura, Mazda and aftermarket bus families are sourced/bounded; P2 expansion is non-blocking and exact protocols remain hardware/capture-gated;
