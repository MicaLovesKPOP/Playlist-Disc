...
`test-kit.json` also records the byte size and SHA-256 of every `beacon.wav`, `disc.toc`, and `manifest.json`, plus one `content_sha256` over the complete test-kit metadata. `verify-test-kit` checks those fingerprints in addition to the semantic `verify-build` checks. This detects byte drift that may still decode to the same PD identity and gives physical compatibility work a stable way to confirm that two test kits were generated identically.

CI builds the complete kit twice and requires every generated byte, including `test-kit.json`, to match. The fingerprint is a reproducibility/provenance aid, not a cryptographic signature: a deliberately edited kit and manifest are not authenticated by it.
