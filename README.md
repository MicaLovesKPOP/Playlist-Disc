...
Draft 0.2 includes a structured compatibility schema and deterministic export so eventual physical testing produces comparable evidence instead of free-form anecdotes. The three reference profiles (Alfa 147 ALFA-937 SW3.17, MINI R55 Boost CD, and Citroen C3 RD4) remain deliberately `planned`; they are architectural anchors rather than physical-compatibility claims. The bounded P1 pre-hardware family sweep is now complete across major European OEM, Japanese OEM, proprietary multimedia, CAN/vehicle-network and aftermarket changer-bus families; P2 expansion is intentionally non-blocking. Physical Draft 0.2 reports must still use development/test IDs and record the exact verified mastering-bundle SHA-256. See `docs/COMPATIBILITY-COVERAGE.md` and `docs/COMPATIBILITY-TESTING.md`.
