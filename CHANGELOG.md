...
- Normalized malformed/truncated WAV parser failures into beacon decode errors so optional beacon evidence remains fail-open in runtime recognition and corrupted build artifacts fail verification cleanly.
...
- Hardened catalog discovery so orphaned/misplaced canonical-manifest JSON and nested/misplaced pack YAML cannot escape whole-catalog validation.
...
- Hardened transcript state handling so non-increasing sequences, rejected capability mutations, unadvertised selection evidence, and reused selection counters report errors without replacing accepted capability/sequence/selection state; only semantically accepted messages advance the sequence baseline, preventing one bad event from masking or causing later violations.
